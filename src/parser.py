...
def parse_args():
    ...
    parser.add_argument('--regs', nargs='?', default='[1e-5,1e-5,1e-2]', help='Regularizations.')  # utilisé
    parser.add_argument('--embed_size', type=int, default=64, help='Embedding size.')
    parser.add_argument('--weight_size', nargs='?', default='[64, 64]', help='Output sizes of every layer') # pas utilisé
    parser.add_argument('--early_stopping_patience', type=int, default=7, help='Early Stop Patience')
    parser.add_argument('--mess_dropout', nargs='?', default='[0.1, 0.1]',help='Keep probability w.r.t. message dropout (i.e., 1-dropout_ratio) for each deep layer. 1: no dropout.') # pas utilisé
    ...
    parser.add_argument('--test_flag', nargs='?', default='part', help='Specify the test type from {part, full}, indicating whether the reference is done in mini-batch') # a tester : done
    ...
    parser.add_argument('--drop_rate', type=float, default=0.0, help='Dropout rate') # a tester
    parser.add_argument('--mask_rate', type=float, default=0.0, help='Mask rate')   # a tester
    parser.add_argument('--mask', type=bool, default=False, help='If mask')  # a tester
    parser.add_argument('--user_cat_rate', type=float, default=2.8, help='User cat rate') # a tester
    parser.add_argument('--item_cat_rate', type=float, default=0.005, help='Item cat rate') # a tester
    parser.add_argument('--model_cat_rate', type=float, default=0.02, help='Model cat rate') # a tester
    ...
    # loss
    parser.add_argument('--aug_mf_rate', type=float, default=0.012, help='Augmentation mf rate')  # a tester
    parser.add_argument('--prune_loss_drop_rate', type=float, default=0.71, help='Prune loss drop rate')   # a tester
    parser.add_argument('--mm_mf_rate', type=float, default=0.0001, help='MM mf rate')
    parser.add_argument('--feat_loss_type', default="sce", type=str, help='Feature loss type')  # a tester
    ...
    parser.add_argument('--aug_sample_rate', type=float, default=0.01, help='Augmentation sample rate') # a tester

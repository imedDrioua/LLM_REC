...
def llm_user_item_interaction(user_history, candidates, llm):
    ...
    prompt = f"""The user has read the following books,that each book with [index] title, year, genres. :
            {user_history}
            Candidates:
            {candidates} Task: The recommendation system needs to predict which books the user will like and which 
            books will dislike from the provided candidates based on the history and analysis. Please provide the 
            output in the format of [index of one liked, index of one disliked] with no introductions or 
            acknowledgments."""
